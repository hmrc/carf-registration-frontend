...
class CheckYourAnswersController @Inject() (
  ...
  private def subscribeAndEnrol()(implicit request: DataRequest[AnyContent], ec: ExecutionContext) = {
      for {
        safeId <- ResultT.fromFuture {
          registrationService.getSafeId(request.userAnswers).map(Right(_))
        }
        ...
        result <- ResultT.fromFuture {
          ...
          sessionRepository.set(updatedUserAnswers).map { _ =>
            Right[ApiError, Result](Redirect(controllers.routes.RegistrationConfirmationController.onPageLoad()))
          }
        }
      } yield result
  }
