...
trait BasicAuthAction {
  def apply(): ActionBuilder[BasicAuthRequest, AnyContent] with ActionFunction[Request, BasicAuthRequest]
}
...
class BasicAuthActionImpl @Inject() (
  ...
  override def apply(): ActionBuilder[BasicAuthRequest, AnyContent] with ActionFunction[Request, BasicAuthRequest] =
    new BasicAuthActionExtractor(authConnector, config, parser)

}

class BasicAuthActionExtractor @Inject() (
    override val authConnector: AuthConnector,
    config: FrontendAppConfig,
    val parser: BodyParsers.Default
)(implicit val executionContext: ExecutionContext)
  ...
  override def invokeBlock[A](
      request: Request[A],
      block: BasicAuthRequest[A] => Future[Result]
  ): Future[Result] = {
