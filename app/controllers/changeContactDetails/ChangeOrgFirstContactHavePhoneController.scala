...
import pages.changeContactDetails.{ChangeDetailsOrgFirstHavePhonePage, ChangeDetailsOrgFirstNamePage, ChangeDetailsOrgFirstPhoneNumberPage}
...
class ChangeOrgFirstContactHavePhoneController @Inject() (
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (carfIdRetrieval() andThen changeDetailsDataRequiredAction) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ChangeDetailsOrgFirstHavePhonePage).fold(form)(form.fill)
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            val oldValue = request.userAnswers.get(ChangeDetailsOrgFirstHavePhonePage)
  ...
  private def handleRedirectionAndUpdateUserAnswers(
    ...
    (oldValue, newValue) match {
      case (Some(true), true) =>
        ...
        } yield Redirect(navigator.nextPage(ChangeDetailsOrgFirstHavePhonePage, mode, updatedAnswers))
      case (_, true)          =>
        for {
          updatedAnswers <- Future.fromTry(request.userAnswers.set(ChangeDetailsOrgFirstHavePhonePage, newValue))
          _              <- sessionRepository.set(updatedAnswers)
      ...
      case _                  =>
        for {
          removedPhone   <- Future.fromTry(request.userAnswers.remove(ChangeDetailsOrgFirstPhoneNumberPage))
          updatedAnswers <- Future.fromTry(removedPhone.set(ChangeDetailsOrgFirstHavePhonePage, newValue))
          _              <- sessionRepository.set(updatedAnswers)
        } yield Redirect(navigator.nextPage(ChangeDetailsOrgFirstHavePhonePage, mode, updatedAnswers))
    }
}
