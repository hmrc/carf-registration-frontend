...
case object OrganisationRegistrationTypePageForNavigatorAndCleanup extends QuestionPage[RegistrationType] {
  ...
  private val soleTraderPages = {
    val indWithUtrPages        = List(WhatIsYourNamePage, IsThisYourBusinessPage)
    val indContactDetailsPages = List(IndividualEmailPage, IndividualHavePhonePage, IndividualPhoneNumberPage)
    indWithUtrPages ++ indGeneralPage ++ indWithNinoPages ++ indWithoutIdPages ++ indContactDetailsPages
  }
    ...
    if (hasChanged) {
      if (newValue == SoleTrader) {
        ...
        throw new IllegalArgumentException(
          "Registration type from /organisation-registration-type cannot be Individual"
        )
      } else {
