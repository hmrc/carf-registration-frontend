...
import pages.{FirstContactNamePage, FirstContactPhonePage}
...
class FirstContactPhoneControllerSpec extends SpecBase with MockitoSugar {
  ...
  "FirstContactPhone Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, firstContactPhoneRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = userAnswersWithName.set(FirstContactPhonePage, true).success.value
      ...
      running(application) {
        ...
        status(result)          mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), NormalMode, "Timothy")(
          request,
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, firstContactPhoneRoute)
