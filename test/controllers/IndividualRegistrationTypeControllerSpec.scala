...
import models.{NormalMode, IndividualRegistrationType, UserAnswers}
...
class IndividualRegistrationTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IndividualRegistrationType Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(IndividualRegistrationTypePage, IndividualRegistrationType.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(IndividualRegistrationType.values.head), NormalMode)(request, messages(application)).toString
      }
    }
