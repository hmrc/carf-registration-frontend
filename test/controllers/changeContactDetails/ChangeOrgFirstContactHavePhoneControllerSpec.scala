...
class ChangeOrgFirstContactHavePhoneControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val changeFirstContactHavePhoneRoute: String =
    controllers.changeContactDetails.routes.ChangeOrgFirstContactHavePhoneController.onPageLoad(NormalMode).url
  ...
  "ChangeFirstContactHavePhone Controller" - {
    ...
    "must return OK and the correct view for a GET when name is provided" in {
      ...
      val application = applicationBuilder(userAnswers =
        Some(emptyUserAnswers.withPage(ChangeDetailsOrgFirstNamePage, "Timmy"))
      ).build()
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId)
        ...
        .withPage(ChangeDetailsOrgFirstNamePage, "Timmy")
    ...
    "must redirect according to the navigator when the existing value is yes and the new value is yes" in {
      ...
      val userAnswers = emptyUserAnswers.withPage(ChangeDetailsOrgFirstHavePhonePage, true)
      val application =
    ...
    "must redirect according to the navigator when the existing value is no and the new value is no" in {
      ...
      val userAnswers = emptyUserAnswers.withPage(ChangeDetailsOrgFirstHavePhonePage, false)
      val application =
        applicationBuilder(userAnswers = Some(userAnswers))
          .overrides(
            bind[Navigator].toInstance(new FakeNavigator(onwardRoute))
