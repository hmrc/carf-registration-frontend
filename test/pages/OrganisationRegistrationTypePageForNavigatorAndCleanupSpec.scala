...
class OrganisationRegistrationTypePageForNavigatorAndCleanupSpec extends SpecBase {
  ...
  "OrganisationRegistrationTypePageForNavigatorAndCleanupSpec" - {
    "cleanup" - {
      "must clear answers" - {
        "when the new answer is different to the previous one and is sole trader" in {
          val ua     = generateUserAnswers(SoleTrader)
            .withPage(OrganisationRegistrationTypePageForNavigatorAndCleanup, SoleTrader)
          val result =
        ...
        "when the new answer is different to the previous one and is Limited Company (non sole trader)" in {
          val ua     = generateUserAnswers(LimitedCompany)
            .withPage(OrganisationRegistrationTypePageForNavigatorAndCleanup, LimitedCompany)
          val result = OrganisationRegistrationTypePageForNavigatorAndCleanup
  ...
  def generateUserAnswers(cleanupType: RegistrationType): UserAnswers = {
    val answers = cleanupType match {
      ...
      case Individual => fail("Registration type cannot be Individual")
