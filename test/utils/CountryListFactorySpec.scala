...
class CountryListFactorySpec extends SpecBase {
  ...
  "countryListWithUKCountries" - {
    "return countries in the UK" in {
      ...
      val factory = new CountryListFactory(
        app.environment,
        new FrontendAppConfig(app.configuration, app.injector.instanceOf[ServicesConfig])
      )

      forAll(factory.countryListWithUKCountries.value) { country =>
  ...
  "countryListWithoutUKCountries" - {
    "return countries not in the UK" in {
      ...
      forAll(factory.countryListWithoutUKCountries.value) { country =>
  ...
  "countryListWithoutGB" - {
    "return countries with country code excluding GB" in {
      ...
      forAll(factory.countryListWithoutGB.value) { country =>
        country.code must not be "GB"
      }
    }
  }
    ...
    "must return non-breaking-space item when no country is selected" in {
      ...
      factory.countrySelectList(Map.empty, countries) must contain theSameElementsAs Seq(
        ...
        SelectItem(
          ...
          text = "Country_1",
          selected = false,
          attributes = Map("data-text" -> "Country_1")
        ),
        SelectItem(
          value = Some("BC"),
          text = "Country_2",
          selected = false,
          attributes = Map("data-text" -> "Country_2")
        )
      )
    }
    ...
    "must return selected country when there is one" in {
      ...
      factory.countrySelectList(selectedCountry, countries) must contain theSameElementsAs Seq(
        ...
        SelectItem(
          ...
          text = "Country_1",
          selected = false,
          attributes = Map("data-text" -> "Country_1:Country_1_2")
        ),
        SelectItem(
          value = Some("BC"),
          text = "Country_2",
